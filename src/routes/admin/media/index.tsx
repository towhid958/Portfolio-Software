...
function MediaLibraryPage() {
  ...
  const { pageItems: pagedMedia, page, setPage, totalPages, total, pageSize } = usePagination(filteredMedia, 24);

  const handleExport = () => {
    exportToCSV(`media-${format(new Date(), 'yyyy-MM-dd')}`, filteredMedia.map((item) => ({
      name: item.name,
      type: item.file_type || '',
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" className="gap-2" onClick={handleExport} disabled={filteredMedia.length === 0}>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-muted-foreground">Try uploading your first asset or changing your search.</p>
            </div>
          ) : (
            <>
              {view === 'grid' ? (
                  ...
                  {pagedMedia.map((item) => (
                    <div key={item.id} className="group relative flex flex-col gap-2">
                      ...
                      <div className="px-1">
                        ...
                        <p className="text-[10px] text-muted-foreground">{formatSize(item.file_size || 0)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <tbody>
                      {pagedMedia.map((item) => (
                        <tr key={item.id} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                          ...
                          <td className="p-4 text-muted-foreground">{formatSize(item.file_size || 0)}</td>
