...
import {
  ...
  InvoiceItemEditor,
} from '@/components/admin/invoices/InvoiceEditors';
...
import { asBillingTo, asEmailStatus } from '@/lib/invoice-json';

/** An invoice as this page's own select returns it. */
...
function AdminInvoices() {
  ...
  const handleResendEmail = async (invoiceId: string) => {
    ...
    try {
      ...
      toast.error('Failed to resend email: ' + getErrorMessage(err));
    } finally {
  ...
  const handleRefund = async (invoiceId: string) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          result.manual
            ? 'Manual refund adjustment recorded'
      ...
      toast.error('Refund error: ' + getErrorMessage(err));
    } finally {
      setRefundingId(null);
  ...
  const handleBatchDownload = async () => {
    ...
    try {
      ...
      toast.error('Batch download failed: ' + getErrorMessage(err));
    } finally {
  ...
  const filteredInvoices = invoices?.filter(
    (inv) =>
      ...
      (inv.invoice_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
        asBillingTo(inv.billing_to)?.name?.toLowerCase().includes(searchTerm.toLowerCase())),
  );
  ...
  const handleExport = () => {
    exportToCSV(
      ...
      (filteredInvoices ?? []).map((inv) => ({
        ...
        customer: asBillingTo(inv.billing_to)?.name || '',
        email: asBillingTo(inv.billing_to)?.email || '',
  ...
  return (
    <>
                ...
                <tbody className="divide-y">
                  {isLoading ? (
                    ...
                    pagedInvoices.map((invoice) => (
                      <tr
                        key={invoice.id}
                        className={`hover:bg-muted/30 transition-colors ${selectedIds.includes(invoice.id) ? 'bg-primary/5' : ''}`}
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="font-medium">{asBillingTo(invoice.billing_to)?.name}</div>
                          <div className="text-xs text-muted-foreground">
                            {asBillingTo(invoice.billing_to)?.email}
                          </div>
                        </td>
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  ...
                                  >
                                    <Mail
                                      className={`h-4 w-4 ${isSendingEmail === invoice.id ? 'animate-pulse' : ''}`}
                                    />
                                    {invoice.last_email_sent_at && (
                                      <div className="absolute -top-1 -right-1">
                                        {asEmailStatus(invoice.last_email_status)?.success ? (
                                  ...
                                  <div className="text-xs space-y-1">
                                    ...
                                    {invoice.last_email_sent_at ? (
                                      <>
                                        <p>
                                          Last sent:{' '}
                                          {format(new Date(invoice.last_email_sent_at), 'MMM d, p')}
                                        </p>
                                        <p>
                                          Status:{' '}
                                          {asEmailStatus(invoice.last_email_status)?.success
                                        ...
                                        {asEmailStatus(invoice.last_email_status)?.error && (
                                          <p className="text-destructive">
                                            Error: {asEmailStatus(invoice.last_email_status)?.error}
                                          </p>
                                        )}
                                      </>
                                    ) : (
                            ...
                            {can('orders', 'edit') && (
                              <>
                                ...
                                <Button
                                  ...
                                  disabled={
                                    refundingId === invoice.id ||
                                    (!!invoice.status &&
                                      ['refunded', 'void'].includes(invoice.status))
                                  }
                                  title="Process Refund"
                                >
                                  <RotateCcw
                                    className={`h-4 w-4 ${refundingId === invoice.id ? 'animate-spin' : ''}`}
                                  />
                                </Button>
                              </>
                            )}
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                For Stripe payments, this attempts a real refund through Stripe for the amount
                below. For manual payments (bank/bKash), it records the adjustment in the system.
                Reduce the amount for a partial refund.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                <Label htmlFor="refund-amount">
                  Refund Amount ({refundTargetInvoice?.currency || 'USD'})
                </Label>
                <Input
                  id="refund-amount"
                  type="number"
                ...
                {refundTargetInvoice && (
                  <p className="text-xs text-muted-foreground">
                    Invoice total: ${refundTargetInvoice.total_amount}{' '}
                    {refundTargetInvoice.currency}
                  </p>
                )}
              </div>
              <div className="space-y-2">
      ...
      <Dialog
        open={!!previewingEmailInvoice}
        onOpenChange={(open) => !open && setPreviewingEmailInvoice(null)}
      >
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="flex items-center gap-3 p-4 bg-muted/50 rounded-lg animate-pulse">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">{currentlyProcessing}</p>
                <p className="text-xs text-muted-foreground italic">Please keep this tab open...</p>
              </div>
            </div>
          </div>
        </DialogContent>
