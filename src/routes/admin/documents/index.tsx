...
function AdminDocumentsPage() {
  ...
  const [previewDoc, setPreviewDoc] = useState<ClientDocumentRow | null>(null);
    ...
    mutationFn: async ({
      ...
      const { data: updatedDoc, error } = await supabase
        ...
        .update({
          ...
          file_type: type,
          metadata: {
            ...(asDocumentMetadata(doc.metadata) || {}),
  ...
  const {
    ...
    total,
    pageSize,
  } = usePagination(filteredDocs);

  const handleExport = () => {
    exportToCSV(
      `documents-${format(new Date(), 'yyyy-MM-dd')}`,
      (filteredDocs ?? []).map((doc) => ({
        title: doc.title,
        client: doc.profiles?.full_name || doc.profiles?.email || '',
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage and provide secure documents to your clients.
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={(filteredDocs?.length ?? 0) === 0}
          >
                      ...
                      <td className="px-6 py-4 text-right space-x-1">
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              const { signedUrl } = await fetchSecureUrl({
        ...
        <DialogContent className="max-w-4xl h-[90vh] flex flex-col p-0 overflow-hidden">
            ...
            <div className="flex items-center justify-between w-full pr-8">
              <div>
                ...
                <CardDescription>
                  ...
                  Uploaded:{' '}
                  {previewDoc?.created_at && format(new Date(previewDoc.created_at), 'PPP')}
                </CardDescription>
              </div>
            </div>
          </DialogHeader>
          <div className="flex-1 bg-muted/20 relative overflow-hidden">
            {/* previewUrl is only ever set alongside previewDoc; stating both
                keeps the branch honest rather than assuming the pairing. */}
          ...
          <DialogFooter className="p-4 border-t bg-muted/5 sm:justify-between items-center">
            <div className="text-xs text-muted-foreground hidden sm:block">
              Type: {previewDoc?.file_type} • Size:{' '}
              {previewDoc?.file_size
                ? `${(previewDoc.file_size / (1024 * 1024)).toFixed(2)} MB`
                : 'N/A'}
            </div>
