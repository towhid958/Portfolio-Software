...
function BlogListingPage() {
  const { category, q } = useSearch({ from: '/blog/' });

  const { data: posts, isLoading } = useQuery({
    queryKey: ['blog-posts', category, q],
    queryFn: async () => {
      let query = supabase
        .from('blog_posts')
        .select('*')
    ...
    <div className="min-h-screen bg-background pt-24 pb-20">
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-wrap gap-2 mb-12">
          ...
          >
            <Link to="/blog" search={{ category: undefined, q }}>
              All Posts
            </Link>
          </Button>
          {categories.map((cat) => (
            <Button
              ...
              className="rounded-full"
            >
              <Link to="/blog" search={{ category: cat as string, q }}>
                {cat as string}
              </Link>
            </Button>
          ))}
