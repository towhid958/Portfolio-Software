...
export function PackageTierEditor({ packages, onChange, onRemove }: PackageTierEditorProps) {
  ...
  const getPkg = (tier: string): PackageData => {
    ...
    return {
      ...
      price: typeof pkg.price === 'number' ? pkg.price : parseFloat(String(pkg.price)) || 0,
      revisions:
        typeof pkg.revisions === 'number' ? pkg.revisions : parseInt(String(pkg.revisions)) || 0,
    };
  };
  ...
  const getErrors = (tier: string) => {
    ...
    if (!pkg.delivery_time.trim()) errors.delivery_time = 'Delivery time is required';
    if (pkg.revisions < 0) errors.revisions = 'Revisions cannot be negative';
    if (pkg.features.length === 0) errors.features = 'At least one feature is required';
    if (pkg.features.some((f) => !f.trim())) errors.features = 'Features cannot be empty';
      ...
      <AlertDialog open={!!tierToDelete} onOpenChange={(open) => !open && setTierToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the <strong>{tierToDelete}</strong> pricing tier from this gig. If
              you have already saved this gig, the package will be permanently deleted from the
              database upon clicking "Save Gig".
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
              ...
              {!packages[tier] ? (
                <div className="py-6 flex flex-col items-center justify-center text-center space-y-2 border-2 border-dashed rounded-lg">
                  ...
                  <p className="text-sm font-medium text-muted-foreground">
                    This tier is currently empty or has been removed.
                  </p>
                  <Button
                    type="button"
                    variant="outline"
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      value={getPkg(tier).name || ''}
                      onChange={(e) =>
                        onChange(tier, { ...getPkg(tier), name: e.target.value || '' })
                      }
                  ...
                  <div className="space-y-2">
                    <Label className={cn(getErrors(tier).price && 'text-destructive')}>
                      Price ($)
                    </Label>
                    <Input
                      type="number"
                      value={getPkg(tier).price || 0}
                      onChange={(e) =>
                        onChange(tier, { ...getPkg(tier), price: parseFloat(e.target.value) || 0 })
                      }
                      className={cn(
                        getErrors(tier).price &&
                  ...
                  <div className="space-y-2">
                    <Label className={cn(getErrors(tier).delivery_time && 'text-destructive')}>
                      Delivery Time
                    </Label>
                    <Input
                      value={getPkg(tier).delivery_time || ''}
                      onChange={(e) =>
                        onChange(tier, { ...getPkg(tier), delivery_time: e.target.value })
                      }
                      placeholder="e.g. 3 Days Delivery"
                      className={cn(
                        getErrors(tier).delivery_time &&
                          'border-destructive focus-visible:ring-destructive',
                      )}
                    />
                  ...
                  <div className="space-y-2">
                    <Label className={cn(getErrors(tier).revisions && 'text-destructive')}>
                      Revisions
                    </Label>
                    <Input
                      type="number"
                      value={getPkg(tier).revisions || 0}
                      onChange={(e) =>
                        onChange(tier, {
                          ...getPkg(tier),
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label className={cn(getErrors(tier).features && 'text-destructive')}>
                        Features
                      </Label>
                      <Button
                        type="button"
                    ...
                    <div className="space-y-2">
                      {getPkg(tier).features.map((feature, index) => (
                          ...
                          <div className="flex gap-2">
                            ...
                            <div className="flex gap-1 shrink-0">
                              ...
                              <Button
                                ...
                                disabled={index === getPkg(tier).features.length - 1}
                              >
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      value={getPkg(tier).cta_text || ''}
                      onChange={(e) =>
                        onChange(tier, { ...getPkg(tier), cta_text: e.target.value })
                      }
