...
import { serviceSchema, type ServiceValues, type ServiceFormValues } from '@/lib/validations';
...
/** The boolean display toggles, rendered from a list below. */
type ToggleField =
  | 'show_packages'
  | 'show_portfolio'
  | 'show_case_studies'
...
interface ServiceFormProps {
  /** An existing row when editing, plus the linked gig ids loaded alongside it. */
  initialData?: (ServiceRow & { package_ids?: string[] }) | undefined;
...
export function ServiceForm({ initialData, onSuccess }: ServiceFormProps) {
  ...
  >('idle');
  // <input, context, output>: the resolver transforms on submit, so the
  // handler receives ServiceValues while the fields hold ServiceFormValues.
  const form = useForm<ServiceFormValues, unknown, ServiceValues>({
    ...
    defaultValues: {
      ...
      status: (initialData?.status as ServiceFormValues['status']) || 'draft',
      ...
      features: (initialData?.features as ServiceFormValues['features']) || [],
      benefits: (initialData?.benefits as ServiceFormValues['benefits']) || [],
      technologies: (initialData?.technologies as ServiceFormValues['technologies']) || [],
      process: (initialData?.process as ServiceFormValues['process']) || [],
      budget_options: (initialData?.budget_options as ServiceFormValues['budget_options']) || [],
      timeline_options: (initialData?.timeline_options ||
        []) as ServiceFormValues['timeline_options'],
      keywords: (initialData?.keywords as ServiceFormValues['keywords']) || [],
      package_ids: initialData?.package_ids || [],
    },
  });
  ...
  const show_packages = watch('show_packages') ?? false;
  ...
  const mutation = useMutation({
    mutationFn: async (values: ServiceValues) => {
      const dbValues = {
        ...
        features: values.features as unknown as Json,
        benefits: values.benefits as unknown as Json,
        technologies: values.technologies as unknown as Json,
        process: values.process as unknown as Json,
        ...
        budget_options: values.budget_options as unknown as Json,
        timeline_options: values.timeline_options as unknown as Json,
        meta_title: values.meta_title,
        meta_description: values.meta_description,
        og_image: values.og_image,
        keywords: values.keywords as unknown as Json,
      };

      let serviceId = initialData?.id;

      if (serviceId) {
        const { error } = await supabase.from('services').update(dbValues).eq('id', serviceId);
        if (error) throw error;
      } else {
        const { data, error } = await supabase.from('services').insert(dbValues).select().single();
        if (error) throw error;
        serviceId = data.id;
      }
      ...
      await logActivity('services', initialData?.id ? 'update_service' : 'create_service', {
        id: serviceId,
        title: values.title,
        slug: values.slug,
      });
    ...
    onError: (error: unknown) => {
      ...
      toast.error(getErrorMessage(error, 'Could not save the service'));
    },
  });
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-1">
                  <SlugField
                    ...
                    value={slug || ''}
                    onChange={(v) =>
                      setValue('slug', v, { shouldValidate: true, shouldDirty: true })
                    }
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={status}
                  onValueChange={(v) =>
                    setValue('status', v as ServiceFormValues['status'], { shouldDirty: true })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="space-y-2">
                ...
                <RichTextEditor
                  ...
                  onChange={(html) =>
                    setValue('full_description', html, { shouldValidate: true, shouldDirty: true })
                  }
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent>
                <MediaPicker
                  value={icon_image}
                  onChange={(url) => setValue('icon_image', url, { shouldDirty: true })}
                />
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {(
                ...
                ] satisfies ReadonlyArray<{ id: ToggleField; label: string }>
              ).map((item) => (
                <div key={item.id} className="flex items-center justify-between py-2">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => setValue(item.id, checked, { shouldDirty: true })}
                  />
                </div>
              ))}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {featureFields.map((field, index) => (
                <div
                  key={field.id}
                  className="grid gap-4 items-start border p-4 rounded-lg relative"
                >
                  <Button
                    type="button"
                    variant="ghost"
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  appendProcess({ step: `${processFields.length + 1}`, title: '', description: '' })
                }
              >
                ...
                >
                  <div className="absolute top-2 right-2 flex gap-1">
                    <Button
                      ...
                      size="icon"
                      disabled={index === 0}
                      onClick={() => moveProcess(index, index - 1)}
                    >
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      disabled={index === processFields.length - 1}
                      onClick={() => moveProcess(index, index + 1)}
                    >
              ...
              <div className="flex items-center justify-between py-2 border-b">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">
                    If enabled, selected packages will appear below service info.
                  </p>
                </div>
                <Switch
                  checked={show_packages}
                  onCheckedChange={(checked) =>
                    setValue('show_packages', checked, { shouldDirty: true })
                  }
                />
              </div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (e.target.checked) {
                              ...
                              setValue(
                                'package_ids',
                                current.filter((id: string) => id !== gig.id),
                                { shouldDirty: true },
                              );
                            }
                          }}
