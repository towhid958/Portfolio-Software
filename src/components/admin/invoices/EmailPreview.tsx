...
export function EmailPreview({ invoiceId, onSend, isSending }: EmailPreviewProps) {
  ...
  useEffect(() => {
    ...
    fetchPreview();
    // getPreview is useServerFn's useCallback over [router, serverFn], both
    // stable, so including it satisfies the rule without re-fetching.
  }, [invoiceId, activeTab, getPreview]);

  // Narrow the Json column once rather than at each access site.
  const emailStatus = asEmailStatus(lastEmail?.last_email_status);
    ...
    <div className="space-y-4 py-4">
        ...
        <div className="border rounded-md p-3 bg-muted/20">
          <div className="flex items-center justify-between mb-2">
            ...
            <Badge variant={emailStatus?.success ? 'default' : 'destructive'}>
              {emailStatus?.success ? 'Success' : 'Failed'}
            </Badge>
          </div>
          <div className="text-xs space-y-1 text-muted-foreground">
            ...
            <div className="flex justify-between">
              <span>Type:</span>
              <span>{emailStatus?.type}</span>
            </div>
            {emailStatus?.error && (
              <div className="text-destructive mt-1 italic">Error: {emailStatus?.error}</div>
            )}
          </div>
        </div>
      )}
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-md flex items-start gap-3">
        ...
        <p className="text-xs text-blue-700 dark:text-blue-300">
          This is a live preview of what the customer will receive. Placeholders like customer name
          and amounts are populated from the current invoice data.
        </p>
      </div>
    </div>
  );
