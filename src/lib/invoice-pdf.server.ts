...
import { asBillingTo, asInvoiceItems } from '@/lib/invoice-json';
...
export async function generateInvoicePDFBuffer(invoiceId: string) {
  ...
  const { data: settings } = await supabaseAdmin.from('invoice_settings').select('*').single();
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(0, 0, 0);
  const billingName = asBillingTo(invoice.billing_to)?.name || 'Valued Client';
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(100, 100, 100);
  doc.text(asBillingTo(invoice.billing_to)?.email || '', margin, y);
  ...
  const items = asInvoiceItems(invoice.items) ?? [];
  items.forEach((item) => {
    doc.setFont('helvetica', 'normal');
  ...
  doc.text(`$${invoice.total_amount} ${invoice.currency}`, pageWidth - margin - 5, y, {
    align: 'right',
  });
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(100, 100, 100);
  const notes = invoice.notes || 'Please complete payment via Stripe or Bank Transfer.';
