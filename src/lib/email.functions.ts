...
export const previewInvoiceEmail = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const customerEmail = asBillingTo(invoice.billing_to)?.email;
    const customerName = asBillingTo(invoice.billing_to)?.name || 'Valued Client';
    ...
    if (template) {
      ...
      ({ subject, html } = getFallbackEmailContent(
        data.type,
